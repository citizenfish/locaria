...
BEGIN
     ...
     USING ST_GEOMFROMEWKT(parameters->>'geometry'),
           parameters->'attributes' || jsonb_build_object('acl', acl_var),
           category_id,
           COALESCE (parameters->>'search_date', NOW()::TEXT)::TIMESTAMP;

     --Add to moderation queue if required
    IF moderated_update THEN
        PERFORM add_to_moderation_queue(parameters || jsonb_build_object('fid', ret_var.id, 'table', parameters->>'table'));
    END IF;
