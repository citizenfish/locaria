...
const SearchLocationFiltersNoResults = () => {
	...
	return (
			<Stack>
				<p>You searched for <b>{searchParams.categories}</b> within <b>{searchParams.distance} {searchParams.distanceType}</b> of <b>{currentLocation&&currentLocation.text ? currentLocation.text:'Your location'}</b> and we have not found any matching results.</p>
				{searchParams.distance < 20 &&
					<>
						<p>Would you like to try increasing the distance to </p>
						<Button variant={"outlined"} onClick={()=>{
							dispatch(setDistance(20));
						}}>20 {searchParams.distanceType}</Button>
					</>
				}
