...
Database.prototype.callRest = function(schema,api,action,payload,success,fail) {
	let querysql = 'SELECT ng_rest.rest_api($1,$2,$3,$4::JSON)';
	let qarguments = ['ng_' + schema, api, action, payload];

	this.client.query(querysql, qarguments,
		function (err, result) {
			if (err) {
				fail(err);
			} else {
