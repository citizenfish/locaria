...
BEGIN
    ...
    CREATE TABLE locaria_data.test_acl(nid BIGSERIAL PRIMARY KEY)
    INHERITS(locaria_data.base_table);

    INSERT INTO locaria_data.test_acl(attributes, wkb_geometry, category_id)
    VALUES(jsonb_build_object('title', 'acl one',   'description', jsonb_build_object('type','test', 'text','acl 1'), 'acl', jsonb_build_object('view', jsonb_build_array('1'))), ST_GEOMFROMEWKT('SRID=4326;Point (-3.51714069502677917 50.3977689822299908)'),  category_var),
          ...
          (jsonb_build_object('title', 'acl three', 'description', jsonb_build_object('type','test', 'text','acl 3'), 'acl', jsonb_build_object('owner','acl 3','view', jsonb_build_array('3'))), ST_GEOMFROMEWKT('SRID=4326;Point (-3.53722966326494737 50.37490912182104097)'), category_var),
